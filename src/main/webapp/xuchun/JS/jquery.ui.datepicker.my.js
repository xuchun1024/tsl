 ...
 */
var $targetVal;
var normal_targetVal;                //保存输出文本框的节点
var last_date_arr = new Array();  //保存原始数据
var date_arr = new Array();       //用于维护已选择日期的数组
var $a = $(".ui-datepicker-calendar").find("a");
var passCtrl = true;       			//默认一直为按住Ctrl键
var isedit = false;         				//是否是编辑广告状态，用于是否显示下部修改提交按钮
...
//获取是否是添加页面
/*
if (document.getElementsByName("add_date_re").length > 0) {
	var isadd = document.getElementById("add_date_re").value;
	if (isadd) {
		isedit = false;
	}
}*/
...
//从修改页面中获取从数据库中取得的Ftimes的数据
//并且该函数用于另一个JS文件的调用(order_editor_base.js 925行)
function get_ftimes() {
	ftimes = '';
	if (document.getElementsByName("ftimes").length > 0) {
		ftimes = document.getElementsByName("ftimes")[0].value;
...
//以下部分为 jQuery UI Datepicker 1.8.10 源码
(function($, undefined) {
	...
	function Datepicker() {
		...
		this._defaults = {// Global defaults for all the date picker instances
			passCtrl : true, // 'focus' for popup on focus,
			...
			showButtonPanel : isedit, // True to show button panel, false to not show it
	...
	$.extend(Datepicker.prototype, {
		...
		_updateDatepicker : function(inst) {
			...
			passCtrl = $.datepicker._get(inst, 'passCtrl');

			$(".ui-datepicker-calendar").find("a").each(function() {
				var action = $(this).attr("action");

				if (date_arr.indexOf(action) >= 0) {
					$(this).css({
						background : "#557D1A",
						color : "white"
					});
			...
			if ($target[0].id != $.datepicker._mainDivId && $target.parents('#' + $.datepicker._mainDivId).length == 0 && !$target.hasClass($.datepicker.markerClassName) && !$target.hasClass($.datepicker._triggerClass) && $.datepicker._datepickerShowing && !($.datepicker._inDialog && $.blockUI))
				{
					if(isedit){
						date_arr = [];
						for(var i=0;i<last_date_arr.length;i++){
							date_arr[i] = last_date_arr[i];
						}
						targetVal = parse_date(date_arr);
						print_datenum();
						$(normal_targetVal).val(targetVal);
					}
					$.datepicker._hideDatepicker();
				}
		},
...
//以下部分为对其重构，以实现日期多选
$(function() {
	$.fn.datepickerRefactor = function(options) {//datepickerRefactor
		var opts = $.extend({}, $.fn.datepickerRefactor.defaults, options);
		normal_targetVal  = opts.target;
		var $this = $(this);
		//var $input = ""
		var passShift = false;

		var focusCss = {
			background : opts.focusColor,
			color : 'white'
		};
		var blurCss = {
			background : opts.blurColor
		};
		...
		$("#datepicker_input").datepicker().change(function() {
			...
			//判断是否存在该日期，效率有限，待改进= =#
			if (date_arr.indexOf(willVal) < 0) {
				date_arr.push(willVal);
				targetVal = parse_date(date_arr);
				//$(opts.target).val(willVal);

			} else {
				date_arr.remove(willVal);
		...
		if (ftimes != '') {
			...
			for (var j = 0; j < strs.length - 1; j++) {
				var strs2 = new Array();
				strs2 = strs[j].split("|");
				time_begin = datetotimestamp(strs2[0].substr(0, 10));
				time_end = datetotimestamp(strs2[1].substr(0, 10));
				time_slot = (time_end - time_begin) / 86400;
				var time_slot_arr = new Array();
				time_slot_arr[0] = time_begin;
				for (var i = 0; i < time_slot; i++) {
					time_slot_arr[i + 1] = time_begin + (i + 1) * 86400;
				}
				for (var i = 0; i < time_slot_arr.length; i++) {
					var tamp = new Date(time_slot_arr[i] * 1000);
					var tampdate = tamp.format('yyyy-MM-dd');
					if (date_arr.indexOf(tampdate) < 0) {
						date_arr.push(tampdate);
					}
				}
			}
			targetVal = parse_date(date_arr);
		...
		//单击确定按钮
		$("#sel_dates_btn").click(function() {
			var sel_time_begin = document.getElementsByName("sel_time_begin")[0].value;
			//$("#sel_time_begin")[0].value;//$("#sel_time_begin").attr("value");
			var sel_time_end = document.getElementsByName("sel_time_end")[0].value;
			//$("#sel_time_end")[0].value;//$("#sel_time_end").attr("value");
			if (sel_time_begin == 'yyyy-mm-dd' || sel_time_end == 'yyyy-mm-dd') {
				alert('两个时间段不能为空！');
			} else {
				time_begin = datetotimestamp(sel_time_begin);
				time_end = datetotimestamp(sel_time_end);
		...
		$this.click(function() {
			...
			//	$("#ui-datepicker-div").attr({style:"position: absolute; top: 1526px; width: 34em; left: 350px; z-index: 1; display: block; "});
		});
...
//将日期转化为用于在页面显示的格式
function parse_date(date_arr) {
	var begindate_arr = new Array();
	var enddate_arr = new Array();
	var timestamp_arr = new Array();
	var finaldate_arr = new Array();
	var realdate_arr = new Array();
	//转换为时间戳
	for (var i = 0; i < date_arr.length; i++) {
		var str = date_arr[i];
		var new_str = str.replace(/:/g, '-');
		...
		var arr = new_str.split("-");
		var datum = new Date(Date.UTC(arr[0], arr[1] - 1, arr[2]));
		timestamp_arr[i] = datum.getTime() / 1000;
	}
	//排序并合并日期
	timestamp_arr.sort();
	begindate_arr.push(timestamp_arr[0]);
	if (timestamp_arr.length <= 1) {
		enddate_arr.push(timestamp_arr[0]);
	}
	for (var i = 0; i < timestamp_arr.length - 1; i++) {
		if (timestamp_arr[i + 1] - timestamp_arr[i] > 86400) {
			enddate_arr.push(timestamp_arr[i]);
			begindate_arr.push(timestamp_arr[i + 1]);
		} else {
			if (i + 1 == timestamp_arr.length - 1) {
				enddate_arr.push(timestamp_arr[i + 1]);
			}
		}
	}
	if (begindate_arr.length > enddate_arr.length) {
		enddate_arr.push(begindate_arr[(begindate_arr.length - 1)]);
	}
	var datestring = "";
	var timelist2 = "";
	for (var i = 0; i < begindate_arr.length; i++) {
		var begin = new Date(begindate_arr[i] * 1000);
		var end = new Date(enddate_arr[i] * 1000);
		if (i == 0) {
			$("#Fbegin_date").val(begin.format('yyyy-MM-dd'));
		}
		if (i == begindate_arr.length - 1) {
			$("#Fend_date").val(end.format('yyyy-MM-dd'));
		}
		realdate_arr[i] = begin.format('yyyy-MM-dd') + " ~ " + end.format('yyyy-MM-dd');
		datestring += (realdate_arr[i] + "\r\n");
		timelist2 += (begin.format('yyyy-MM-dd') + " 00:00|" + end.format('yyyy-MM-dd') + " 24:00|0,");
	}
	$("#timelist2").val(timelist2);
	return datestring;
}
...
function modify_date() {
	...
	var Forder_id = '';
	if (document.getElementsByName("order_id").length > 0) {
		Forder_id = document.getElementsByName("order_id")[0].value;
	}
	var Fbegin_date = document.getElementsByName("Fbegin_date")[0].value;
	var Fend_date = document.getElementsByName("Fend_date")[0].value;
	var timelist2 = document.getElementsByName("timelist2")[0].value;
	var Ftimes_control = Fbegin_date + "#" + Fend_date + "#" + timelist2;
	if (isedit) {
		$.post("/order/modify", {
			Forder_id : Forder_id,
			field_name : "Ftimes_control",
			field_value : Ftimes_control
		}, function(data) {
			if (data.ret_code == -1) {
				alert(data.ret_msg);
			} else {
